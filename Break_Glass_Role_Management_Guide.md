...
Here is the calling hierarchy outline of the custom operations flow, subflows, and actions to be created.
...
To prevent having to go back and forth when creating parent and child components in the documentation, this guide will build out the process engine flow backwards; i.e., _Break Glass Get Expirations_, _Break Glass Grant Role Banner_, _Break Glass Grant Role_, _Break Glass Close User Sessions_, and then _Break Glass Roles - Catalog Item_. This will allow the parent to link to the existing child operation component, without having to go back and update parent calling operations.

---
## Break Glass Get Expirations
\
The _Break Glass Get Expirations_ is an action component within the flow operations. The purpose of the action is to determine when the break glass role is expiring and expires. Both of these dates and times are used for banners and used to determine when to revoke the break glass role from the user's account. 
...
\
The _Break Glass Grant Role Banner_ is a subflow component within the flow operations. The purpose of the subflow is to setup notification banners for the user to show that they were granted the role, when the role is expiring and expires. 

The subflow uses the ServiceNow OOTB banner feature. ServiceNow banners lack the ability to target individual accounts. To target banners for specific users, users will need unique roles applied to their accounts. This means each user account will need a unique role. This flow checks for the role u_user_[user id] to exist. Beneath the subflow instructions, are instructions on adding unique roles for user accounts.

Because custom individual roles per user is not standard, the subflow checks the system property break_glass.banner to see if the value is true or false. If true, then banners are used; else banners are not used.
...
The above subflow relies on two additional aspects; a system property and that users have a unique user account role. If you do not plan on using banners with your break glass deployment, then set the value of **break_glass.banner** to **false**.
...
\
The _Break Glass Grant Role_ is a subflow component within the flow operations. The purpose of the subflow is to grant the user the break glass role.
...
1. The following instructions are adding operations under the **Actions** section. Due to the difficulty of documenting flow operations, the following sub-ordered steps represent the order of flow operations. At the start of each action there will either be the choice **Add an Action, Flow Logic, or Subflow** or a grouping of **Action**, **Flow Logic**, and **Subflow**. The step will assume general knowledge of which to select and finalizing the operation by clicking **Done**. 
    ...
    1. (Under ERROR HANDLER) **Flow Logic** > **Assign Subflow Outputs**
        - [Granted] false
        - [Message] An unpected error occurred granting the break glass role **Input** > **Break Glass Role** > **Name** to **Input** > **Request Item** > **Requested for** > **Name**.
1. Click **Save**.
1. Click **Publish**.
...
\
The _Break Glass Close User Sessions_ is an action component within the flow operations. The purpose of the action is to terminate all of the user's active sessions. This will force the user to re-login into ServiceNow and thus be granted the break glass roles. This is also used later on, when automation revokes the break glass role from the user as well.
...
\
\
The _Break Glass Roles - Catalog Item_ is the flow that processes service requests from the _Break Glass Roles_ catalog item. 
...
1. The following instructions are adding operations under the **Actions** section. Due to the difficulty of documenting flow operations, the following sub-ordered steps represent the order of flow operations. At the start of each action there will either be the choice **Add an Action, Flow Logic, or Subflow** or a grouping of **Action**, **Flow Logic**, and **Subflow**. The step will assume general knowledge of which to select and finalizing the operation by clicking **Done**. 
    ...
    1. (Under 13 - If) **Action** > **ServiceNow Core** > **Update Record**
        ...
        - _Fields:_
            ...
            - [Additional comments] Automation detected a filure in granting the break glass roles to the account. Investigate the issue and resolve.
    1. **Flow Logic** > **Else**
...
1. Naviagte to **Service Catalog** > **Catalog Definitions** > **Maintain Items**.
